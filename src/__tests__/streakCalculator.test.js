...
describe("toLocalDateString", () => {
  it("maps UTC timestamps to the correct local calendar day", () => {
    ...
    expect(
      toLocalDateString("2026-01-02T07:55:00Z", "America/Los_Angeles"),
    ).toBe("2026-01-01");
  });
...
describe("calculateGithubStreak", () => {
  it("counts consecutive local days instead of UTC days", () => {
    ...
    expect(
      calculateGithubStreak(events, "America/Los_Angeles", referenceDate),
    ).toBe(2);
  ...
  it("preserves streak when user contributed yesterday in local time", () => {
    ...
    expect(calculateGithubStreak(events, "Asia/Kolkata", referenceDate)).toBe(
      1,
    );
  });
