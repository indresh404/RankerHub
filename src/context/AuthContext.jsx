...
const checkAndUpdateStreak = async (data, docRef) => {
  ...
  try {
    ...
    logger.error("Failed to update streak atomically:", err);
  }
};
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (!auth) {
      logger.warn("Firebase auth is not initialized; auth listener skipped.");
      return undefined;
    }
    ...
    getRedirectResult(auth)
      ...
      .catch((error) => {
        logger.error("Redirect sign-in resolution failure:", error);
      });
  ...
  const login = async (requestRepoScope = true) => {
    try {
      ...
      if (!validation.isValid) {
        logger.warn("User data validation warnings:", validation.errors);
      }
  ...
  const purchaseMascot = async (mascotId, price) => {
    ...
    try {
      ...
      logger.error("Failed to purchase mascot:", err);
      throw err;
    }
  };
  ...
  const equipMascot = async (mascotId) => {
    ...
    try {
      ...
      logger.error("Failed to equip mascot:", err);
      throw err;
    }
  };
  ...
  const fetchGitHubStats = async (uid, username) => {
    ...
    try {
      ...
      try {
        ...
        if (commitsRateLimit) logger.warn(commitsRateLimit);
        else commits = commitsData?.total_count || 0;
      } catch (err) {
        logger.warn("Commits retrieval failed:", err);
      }
      ...
      try {
        ...
        if (prsRateLimit) logger.warn(prsRateLimit);
        else prs = prsData?.total_count || 0;
      } catch (err) {
        logger.warn("PRs retrieval failed:", err);
      }
      ...
      try {
        ...
        if (reviewsRateLimit) logger.warn(reviewsRateLimit);
        else reviews = reviewsData?.total_count || 0;
      } catch (err) {
        logger.warn("Reviews retrieval failed:", err);
      }
      ...
      if (rateLimitMsg) {
        logger.warn("Rate limit hit:", rateLimitMsg);
  ...
  const syncGitHubData = async () => {
    ...
    try {
      ...
      const rateLimitMsg = getRateLimitMessage(error);
      if (rateLimitMsg) {
        logger.warn("Sync blocked by rate limit:", rateLimitMsg);
        throw new Error(rateLimitMsg, { cause: error });
      }
