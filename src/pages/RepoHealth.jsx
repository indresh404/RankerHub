...
export const RepoHealth = () => {
  ...
  const handleScan = async (e, customUrl = null) => {
    ...
    if (!parsed) {
      setError(
        "Please enter a valid GitHub repository URL (e.g., https://github.com/facebook/react) or format (owner/repo).",
      );
      return;
    }
    ...
    const token =
      TokenManager.get(user?.uid) ||
      sessionStorage.getItem(`gh_token_${user?.uid}`);
    ...
    try {
      ...
      const [contributorsRes, releasesRes, pullsRes, issuesRes, commitsRes] =
      ...
      const totalCommitsSum = contributors.reduce(
      ...
      const mergedPRs = closedPRs.filter(
        (p) => p.pull_request?.merged_at || p.merged_at,
      );
      const unmergedClosedPRs = closedPRs.filter(
        (p) => !(p.pull_request?.merged_at || p.merged_at),
      );
      ...
      mergedPRs.forEach((pr) => {
        ...
        const merged = new Date(
          pr.pull_request?.merged_at || pr.merged_at || pr.closed_at,
        );
      ...
      const issueResolutionRate =
        openIssues + closedIssues.length > 0
          ? Math.round(
              (closedIssues.length / (openIssues + closedIssues.length)) * 100,
            )
          : 100;
      ...
      if (releases.length > 0) {
        releases.forEach((rel) => {
          ...
          const relDate = new Date(
            rel.published_at || rel.created_at || new Date(),
          );
      ...
      commits.forEach((commit) => {
        ...
        const weekIdx = weeklyCommits.findIndex(
      ...
      if (releasesLastYear >= 12) releaseScore = 25;
        ...
        const diffMonths =
          (nowTime.getTime() - lastPush.getTime()) / (1000 * 60 * 60 * 24 * 30);
      ...
      const responseData = {
        ...
        topContributors: sortedContributors.slice(0, 5).map((c) => ({
          ...
          percent:
            totalCommitsSum > 0
              ? Math.round((c.contributions / totalCommitsSum) * 100)
              : 0,
        })),
        pr: {
          open: openPRs,
      ...
      if (status === 403 || status === 429) {
        ...
        setError(
          "GitHub API rate limit exceeded. Log in with GitHub or wait before trying again.",
        );
      } else if (status === 404) {
        setError(
          "Repository not found. Please verify the URL or path and check if it is public.",
        );
      } else {
        setError(
          err.message ||
            "Failed to scan repository. Please check your network and try again.",
        );
      }
    } finally {
      setIsScanning(false);
  ...
  const sustainabilityTier = useMemo(() => {
    ...
    if (score >= 90)
      return {
        label: "Excellent - High Sustainability",
        color: "text-emerald-500 bg-emerald-500/10 border-emerald-500/20",
        desc: "Outstanding maintenance, robust collaborative base, and active release schedule.",
      };
    if (score >= 70)
      return {
        label: "Healthy - Moderate Sustainability",
        color: "text-blue-500 bg-blue-500/10 border-blue-500/20",
        desc: "Consistent contributions, good resolution rates, and low developer concentration risk.",
      };
    if (score >= 50)
      return {
        label: "Stagnant - Moderate Risk",
        color: "text-amber-500 bg-amber-500/10 border-amber-500/20",
        desc: "Slow issue/PR responses or high maintainer concentration. Proceed with caution.",
      };
    return {
      label: "Critical - High Risk",
      color: "text-red-500 bg-red-500/10 border-red-500/20",
      desc: "Dependent on a single contributor, low activity levels, or lack of maintenance.",
    };
  }, [healthData]);
  ...
  const commitsPoints = useMemo(() => {
    ...
    return healthData.weeklyCommits.map((d, i) => {
      const x =
        paddingX +
        (i / (healthData.weeklyCommits.length - 1)) *
          (chartWidth - 2 * paddingX);
      const y =
        chartHeight -
        paddingY -
        (d.count / commitsMaxVal) * (chartHeight - 2 * paddingY);
      return { x, y };
    });
  ...
  const commitsPath = useMemo(() => {
    ...
    return (
      `M ${commitsPoints[0].x} ${commitsPoints[0].y} ` +
      commitsPoints
        .slice(1)
        .map((p) => `L ${p.x} ${p.y}`)
        .join(" ")
    ...
    <div className="space-y-8 animate-fade-in pb-20 overflow-x-hidden">
      ...
      <Card className="p-3 sm:p-5 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl border-slate-200/50 dark:border-slate-800/50">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-4 text-xs font-semibold text-slate-400 dark:text-slate-500">
          ...
          {["facebook/react", "vuejs/core", "rust-lang/rust", "golang/go"].map(
            (p) => (
              <button
                key={p}
                type="button"
          ...
          <div>
            ...
            {rateLimitWarning && !user && (
              <p className="mt-2 text-xs text-red-500/80">
                Tip: Authenticated users get 5000 requests/hour from GitHub
                instead of the guest 60 requests/hour. Try logging in!
              </p>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card className="p-6 flex flex-col items-center justify-center text-center lg:col-span-1">
                ...
                >
                  ...
                  <circle
                    className={`transition-all duration-1000 ease-out ${
                      healthData.scores.total >= 90
                        ? "text-emerald-500"
                        : healthData.scores.total >= 70
                          ? "text-blue-500"
                          : healthData.scores.total >= 50
                            ? "text-amber-500"
                            : "text-red-500"
                    }`}
                    strokeWidth="8"
                    ...
                    strokeDashoffset={
                      251.2 - (251.2 * healthData.scores.total) / 100
                    }
                    strokeLinecap="round"
                    stroke="currentColor"
                    fill="transparent"
              ...
              <div className="mt-6 space-y-2">
                ...
                <div
                  className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border ${sustainabilityTier.color}`}
                >
            ...
            <Card className="lg:col-span-2 p-6 flex flex-col justify-between">
              <div>
                ...
                <div className="mt-6 space-y-3.5">
                  {[
                    {
                      label:
                        "Bus Factor / Maintainer Dependency (Commit Concentration)",
                      score: healthData.scores.busFactor,
                      color: "bg-purple-500",
                    },
                    {
                      label: "Issue Resolution Rate (Closed vs Open issues)",
                      score: healthData.scores.issue,
                      color: "bg-blue-500",
                    },
                    {
                      label: "PR Merge Rate (Merged pull requests efficiency)",
                      score: healthData.scores.pr,
                      color: "bg-pink-500",
                    },
                    {
                      label:
                        "Release Frequency & Recency (Longevity of updates)",
                      score: healthData.scores.release,
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800/60 flex flex-wrap items-center justify-between gap-4 text-xs font-semibold text-slate-400 dark:text-slate-500">
                ...
                <span>
                  Last Commit:{" "}
                  {new Date(healthData.pushedAt).toLocaleDateString()}
                </span>
              </div>
            </Card>
          </div>
            ...
            <Card className="p-6 space-y-4">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  ...
                  <h4 className="text-lg font-black text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                    <Users className="w-5 h-5 text-purple-500" /> Bus Factor:{" "}
                    {healthData.busFactor}
                  </h4>
                </div>
                <span
                  className={`px-2.5 py-1 text-[10px] font-bold rounded-full border ${
                    healthData.busFactor >= 5
                      ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20"
                      : healthData.busFactor >= 3
              ...
              {healthData.topContributors.length > 0 && (
                <div className="space-y-3.5 pt-2">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block border-b border-slate-100 dark:border-slate-800/60 pb-1.5">
                    Top Contributors Shares
                  </span>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <img
                            ...
                            alt={contrib.login}
                            className="w-5 h-5 rounded-full ring-1 ring-violet-500/20"
                          />
                        ...
                        <div className="flex items-center gap-3 w-40 justify-end">
                          ...
                          <span className="font-extrabold text-slate-500 text-[10px] w-8 text-right">
                            {contrib.percent}%
                          </span>
                        </div>
                      </div>
            ...
            <Card className="p-6 space-y-4">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  ...
                  <h4 className="text-lg font-black text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                    <GitPullRequest className="w-5 h-5 text-pink-500" /> PR
                    Merge Rate: {healthData.pr.rate}%
                  </h4>
                </div>
                <span
                  className={`px-2.5 py-1 text-[10px] font-bold rounded-full border ${
                    healthData.pr.rate >= 75
                      ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20"
                      : healthData.pr.rate >= 50
                        ? "bg-blue-500/10 text-blue-500 border-blue-500/20"
                        : "bg-amber-500/10 text-amber-500 border-amber-500/20"
                  }`}
                >
              ...
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400 leading-relaxed">
                Represents the percentage of resolved pull requests that
                successfully merged.
                {healthData.pr.avgTime > 0 &&
                  ` On average, accepted PRs take ${healthData.pr.avgTime} days to merge.`}
              </p>
              ...
              <div className="space-y-2 pt-2">
                ...
                <div className="flex h-3 w-full rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800/80">
                  <div
                    ...
                    style={{
                      width: `${(healthData.pr.merged / (healthData.pr.open + healthData.pr.closed)) * 100 || 0}%`,
                    }}
                    title="Merged"
                  />
                  <div
                    ...
                    style={{
                      width: `${(healthData.pr.unmergedClosed / (healthData.pr.open + healthData.pr.closed)) * 100 || 0}%`,
                    }}
                  ...
                  <div
                    ...
                    style={{
                      width: `${(healthData.pr.open / (healthData.pr.open + healthData.pr.closed)) * 100 || 0}%`,
                    }}
                    title="Open"
                  />
                </div>
                ...
                <div className="flex justify-between items-center text-[10px] font-bold text-slate-400 pt-1">
                  <span className="flex items-center gap-1">
                    ...
                    Merged ({healthData.pr.merged})
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full bg-red-500" /> Closed
                    ({healthData.pr.unmergedClosed})
                  </span>
                  <span className="flex items-center gap-1">
              ...
              {healthData.pr.avgTime > 0 && (
                <div className="pt-2 flex items-center gap-2 text-xs font-bold text-slate-400">
                  ...
                  <span>
                    Avg. PR processing time:{" "}
                    <span className="text-slate-600 dark:text-slate-200">
                      {healthData.pr.avgTime} days
                    </span>
                  </span>
                </div>
              )}
            ...
            <Card className="p-6 space-y-4">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  ...
                  <h4 className="text-lg font-black text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                    <Activity className="w-5 h-5 text-blue-500" /> Issue
                    Resolution: {healthData.issue.rate}%
                  </h4>
                </div>
                <span
                  className={`px-2.5 py-1 text-[10px] font-bold rounded-full border ${
                    healthData.issue.rate >= 80
                      ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20"
                      : healthData.issue.rate >= 50
                        ? "bg-blue-500/10 text-blue-500 border-blue-500/20"
                        : "bg-amber-500/10 text-amber-500 border-amber-500/20"
                  }`}
                >
              ...
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400 leading-relaxed">
                ...
                {healthData.issue.avgTime > 0 &&
                  ` Resolved issues are handled in an average of ${healthData.issue.avgTime} days.`}
              </p>
              ...
              <div className="space-y-2 pt-2">
                ...
                <div className="flex h-3 w-full rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800/80">
                  <div
                    className="bg-emerald-500"
                    style={{
                      width: `${(healthData.issue.closed / (healthData.issue.open + healthData.issue.closed)) * 100 || 0}%`,
                    }}
                    title="Closed"
                  />
                  <div
                    className="bg-blue-500 animate-pulse"
                    style={{
                      width: `${(healthData.issue.open / (healthData.issue.open + healthData.issue.closed)) * 100 || 0}%`,
                    }}
                    title="Open"
                  />
                </div>
                ...
                <div className="flex justify-between items-center text-[10px] font-bold text-slate-400 pt-1">
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full bg-emerald-500" />{" "}
                    Closed Issues ({healthData.issue.closed})
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full bg-blue-500" /> Open
                    Issues ({healthData.issue.open})
                  </span>
                </div>
              </div>
            ...
            <Card className="p-6 space-y-4">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  ...
                  <h4 className="text-lg font-black text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                    <Calendar className="w-5 h-5 text-emerald-500" /> Releases
                    (1yr): {healthData.releases.countLastYear}
                  </h4>
                </div>
                <span
                  className={`px-2.5 py-1 text-[10px] font-bold rounded-full border ${
                    healthData.releases.countLastYear >= 12
                      ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20"
                      : healthData.releases.countLastYear >= 4
                        ? "bg-blue-500/10 text-blue-500 border-blue-500/20"
                        : "bg-amber-500/10 text-amber-500 border-amber-500/20"
                  }`}
                >
              ...
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400 leading-relaxed">
                Measures the frequency of official software releases or package
                version updates published to tags.
              </p>

              <div className="space-y-3 pt-2">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block border-b border-slate-100 dark:border-slate-800/60 pb-1.5">
                  Release Timeline Info
                </span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 dark:bg-slate-950/40 p-2.5 rounded-xl border border-slate-200/20 dark:border-slate-800/20 flex flex-col justify-center">
                    ...
                    <span className="text-base font-extrabold text-slate-700 dark:text-slate-200 mt-0.5">
                      {healthData.releases.totalCount}
                    </span>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-950/40 p-2.5 rounded-xl border border-slate-200/20 dark:border-slate-800/20 flex flex-col justify-center">
                    <span className="text-[10px] font-bold text-slate-400 block uppercase">
          ...
          <Card className="p-6">
            <div className="space-y-1">
              <h4 className="font-extrabold text-slate-800 dark:text-slate-200 my-0 flex items-center gap-1.5">
                <TrendingUp className="w-5 h-5 text-indigo-500" /> Commit
                Activity Trend
              </h4>
              <p className="text-[11px] text-slate-400 font-semibold mb-4">
                Repository commit history across the 50 most recent commits
                (mapped into weekly intervals).
              </p>
            </div>
            ...
            <div className="w-full flex items-center justify-center pt-4">
              ...
              >
                ...
                {[0, 0.25, 0.5, 0.75, 1].map((r, i) => {
                  ...
                  const paddingY = 20;
                  const y =
                    chartHeight - paddingY - r * (chartHeight - 2 * paddingY);
                  return (
                    <line
                      key={i}
                      x1={35}
                ...
                {commitsArea && (
                  <path
                    d={commitsArea}
                    className="fill-indigo-500/5 dark:fill-indigo-500/5"
                  />
                )}
